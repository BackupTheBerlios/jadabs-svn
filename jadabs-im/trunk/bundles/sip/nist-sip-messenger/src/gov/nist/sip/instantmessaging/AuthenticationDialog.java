...
public class AuthenticationDialog
{
    ...
    public void okButtonActionPerformed(ActionEvent evt)
    {
        if (userNameTextField.getText() == null || userNameTextField.getText().trim().equals(""))
            new AlertInstantMessaging("You must enter an user name!!!", JOptionPane.ERROR_MESSAGE);
        else
        {
            char[] pass = passwordTextField.getPassword();

            if (pass == null)
                new AlertInstantMessaging("You must enter a password!!!", JOptionPane.ERROR_MESSAGE);
            else
            {
