    ...
    public void registerToProxies()
    {
        try
        {
            ...
            if (proxyToRegisterWithList != null)
            {
                ...
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("Registrar, registerToProxies(), we have to register to "
                            + proxyToRegisterWithList.size() + " proxies");
                }
                for (int i = 0; i < proxyToRegisterWithList.size(); i++)
                {
                    Domain domain = (Domain) proxyToRegisterWithList.elementAt(i);
                    if (domain.hostName != null)
                    {
                        RegistrationDomainThread rr = new RegistrationDomainThread(proxy, domain);
                        //ifdef SIMULATION
                        /*
        ...
        } catch (Exception e)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("ERROR, Registrar, registerToProxies(), exception  raised:");
            }
            e.printStackTrace();
        }
    }
...
//        }
    ...
    public void parseXMLregistrations(String file)
    {
        try
        {
            ...
            XMLRegistrationsParser xmlRegistrationsParser = new XMLRegistrationsParser(xmlRegistrationsFile, proxy);
            ...
            if (registrationList != null)
            {
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("Registrar, parseXMLregistrations(), Uploading of " + registrationList.size()
                            + " registrations");
                }
                for (int i = 0; i < registrationList.size(); i++)
                {
                    Registration registration = (Registration) registrationList.elementAt(i);
        ...
        } catch (Exception e)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("ERROR, Registrar, Registrar(), exception  raised during"
    ...
    public void initRMIBindings()
    {
        ...
        try
        {
            ...
            if (configuration.accessLogViaRMI)
            {
                ...
                name = "//" + stackIPAddress + ":" + 0 + "/" + sipStack.getStackName() + "/"
            ...
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("We don't export the registrations because RMI is disabled.");
            }
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Problem trying to export the Registration Table: " + name);
            }
            ex.printStackTrace();
        }
    }
     ...
     */
    public synchronized void processRegister(Request request, SipProvider sipProvider,
            ServerTransaction serverTransaction)
    {
        try
        {
            ...
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, processRegister(), key: \"" + key + "\"");
            }
            if (key == null)
            {
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("Registrar, processRegister(), key is null" + " 400 INVALID REQUEST replied");
                }
            ...
            if (!hasContactHeaders(request))
            {
                ...
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("Registrar, processRegister(), response sent:" + response.toString());
                }
                return;
            }
            ...
            if (hasStar(request))
            {
                ...
                if (contactHeaders.size() > 1)
                {
                    if (ProxyDebug.debug)
                    {
                        ProxyDebug.println("Registrar, processRegister(), more than one contact header"
                                + " is present at the same time as a wild card." + " 400 INVALID REQUEST replied");
                    }
                    Response response = messageFactory.createResponse(Response.BAD_REQUEST, request);
                    if (serverTransaction != null)
                        serverTransaction.sendResponse(response);
                    ...
                    if (ProxyDebug.debug)
                    {
                        ProxyDebug.println("Registrar, processRegister(), response sent:");
                ...
                if (!hasExpiresZero(request))
                {
                    if (ProxyDebug.debug)
                    {
                        ProxyDebug.println("Registrar, processRegister(), expires time different from"
                                + " 0 with a wild card." + " 400 INVALID REQUEST replied");
                    }
                    Response response = messageFactory.createResponse(Response.BAD_REQUEST, request);
                ...
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("Registrar, processRegister(), (* and expires=0) "
            ...
            if (registrationsTable.hasRegistration(key))
            {
                ...
                if (proxy.getConfiguration().rfc2543Compatible && key.indexOf(":5060") < 0)
                {
                    ...
                    System.out.println("CISCO IP PHONE FIX:  " + "Updating proper registration for " + key);
            ...
            } else
            {
                ...
                if (hasExpiresZero(request))
                {
                    ...
                    ProxyDebug
                            .println("Registrar, processRegister(), " + "we don't have any record for this REGISTER.");
                    Response response = messageFactory.createResponse(Response.OK, request);
                ...
                if (proxy.getConfiguration().rfc2543Compatible && key.indexOf(":5060") < 0)
                {
                    ...
                    System.out.println("CISCO IP PHONE FIX:  " + "adding proper registration for " + key);
        ...
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, processRegister(), internal error, " + "exception raised:");
                ProxyDebug.logException(ex);
            }
        }
    }
    ...
    public String getKey(Request request)
    {
        ...
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, hasDomainRegistered(), internal error, " + "exception raised:");
                ProxyDebug.logException(ex);
            }
            return null;
        }
    ...
    public Vector getDomainContactsURI(Request request)
    {
        ...
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, getDomainContacts(), internal error, " + "exception raised:");
                ProxyDebug.logException(ex);
            }
            return null;
        }
    ...
    public boolean hasRegistration(Request request)
    {
        ...
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, hasRegistration(), internal error, " + "exception raised:");
                ProxyDebug.logException(ex);
            }
            return false;
        }
    ...
    public Vector getContactsURI(Request request)
    {
        ...
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, getContactsURI(), internal error, exception raised:");
                ProxyDebug.logException(ex);
            }
            return null;
        }
    ...
    /*
     * Matches a Sip URI "sip:user@domain" with a list of Contacts @param key
     * The sip URI found in the To-header of a request @author Henrik Leion
     */
    ...
    private boolean hasStar(Request request) throws Exception
    {
        ...
        while (list.hasNext())
        {
            ...
            if (contactHeader.getAddress().isWildcard()) { return true; }
        }
        return false;
    }
    ...
    private boolean hasExpiresZero(Request request)
    {
        try
        {
            ...
            {
                ProxyDebug.println("Registrar, hasExpiresZero(), the REGISTER does not have an Expires Header");
                return false;
            } else
            {
                ProxyDebug.println("Registrar, hasExpiresZero(), the REGISTER has an Expires Header with"
        ...
        } catch (Exception e)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, hasExpiresZero(), internal error, exception raised:");
    ...
    public static Vector getContactHeaders(Request request)
    {
        ...
        } catch (Exception e)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("Registrar, getContactHeaders(), internal error, exception raised:");
                ProxyDebug.logException(e);
            }
            return contacts;
        }
