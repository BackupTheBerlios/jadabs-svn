...
public class Authentication
{
    ...
    public boolean isAuthorized(Request request)
    {
        ...
        if (request.getMethod().equals(Request.ACK) || request.getMethod().equals(Request.CANCEL))
            return true;
        ProxyAuthorizationHeader proxyAuthorization = (ProxyAuthorizationHeader) request
                .getHeader(ProxyAuthorizationHeader.NAME);

        if (proxyAuthorization == null)
        {
            ProxyDebug.println("Authentication failed: ProxyAuthorization header missing!");
            return false;
        }
        ...
        try
        {
            boolean res = authenticationMethod.doAuthenticate(username, proxyAuthorization, request);
            if (res)
                ProxyDebug.println("Authentication passed for user: " + username);
            else
                ProxyDebug.println("Authentication failed for user: " + username);
            return res;
        } catch (Exception e)
        {
            e.printStackTrace();
    ...
    public Response getResponse(Request request)
    {
        try
        {
            ...
            {
                ProxyDebug.println("ERROR, you have to initialize the Authentication class");
                return null;
            }
            ...
            ProxyAuthenticateHeader proxyAuthenticate = headerFactory
                    .createProxyAuthenticateHeader(authenticationMethod.getScheme());

            String realm = authenticationMethod.getRealm(null);
            if (!realm.trim().equals(""))
                proxyAuthenticate.setParameter("realm", realm);

            proxyAuthenticate.setParameter("nonce", authenticationMethod.generateNonce());
