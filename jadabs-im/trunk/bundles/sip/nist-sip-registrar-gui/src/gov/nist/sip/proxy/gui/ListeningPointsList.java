...
import gov.nist.sip.proxy.Association;
...
public class ListeningPointsList extends JList
{
    ...
    public boolean hasListeningPoint(String port, String transport)
    {
        ...
        while (e != null && e.hasMoreElements())
        {
            ...
            if (asso.port.equals(port) && asso.transport.equals(transport)) { return true; }
        }
        return false;
    }
    ...
    public void addListeningPoint(String port, String transport)
    {
        ...
        if (hasListeningPoint(port, transport))
            new AlertFrame("The listening point is already in the list", JOptionPane.ERROR_MESSAGE);
        else
        {
            counter++;
            lps.put("listeningPoint" + counter, new Association(port, transport));
            if (list.size() == 1)
            {
    ...
    public void removeSelectedListeningPoint()
    {
        ...
        {
            new AlertFrame("You must select a listening point to remove!", JOptionPane.ERROR_MESSAGE);
        }
    }
