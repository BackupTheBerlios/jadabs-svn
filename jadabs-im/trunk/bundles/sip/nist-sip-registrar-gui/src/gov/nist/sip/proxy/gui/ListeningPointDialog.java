...
public class ListeningPointDialog
{
    ...
    public boolean checkLPTransport(String transport)
    {
        try
        {
            if (transport == null || transport.trim().equals(""))
            {
                new AlertFrame("Specify a transport parameter!");
                return false;
            }
            if (transport.trim().compareToIgnoreCase("UDP") == 0 || transport.trim().compareToIgnoreCase("TCP") == 0)
                return true;
            else
            {
    ...
    {
        if (checkLPPort(portTextField.getText()) && checkLPTransport(transportTextField.getText()))
