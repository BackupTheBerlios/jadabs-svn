...
public class BuddyList extends JList
{
    ...
    {
        DebugIM.println("DebugIM, BuddyList, init(), There are " + buddies.size() + " buddies to upload.");
        for (int i = 0; i < buddies.size(); i++)
        {
            BuddyTag buddyTag = (BuddyTag) buddies.elementAt(i);
            String buddy = buddyTag.getURI();

            DebugIM.println("DebugIM, BuddyList, init(), we uploaded " + buddy + " to the buddy List.");
            if (!hasBuddy(buddy))
    ...
    public boolean hasBuddy(String bud)
    {
        for (int i = 0; i < buddies.size(); i++)
        {
            ...
            if (buddy.equals(bud))
            {
                DebugIM.println("DebugIM, BuddyList, hasBuddy(), the buddy is" + " in the list");
                return true;
            }
        }
        return false;
    ...
    public BuddyTag getBuddyTag(String bud)
    {
        for (int i = 0; i < buddies.size(); i++)
        {
            BuddyTag buddyTag = (BuddyTag) buddies.elementAt(i);
            String buddy = buddyTag.getURI();
            if (buddy.equals(bud)) { return buddyTag; }
        }
        return null;
    }
    ...
    public void changeBuddyStatus(String buddy, String status)
    {
        try
        {
            ...
            {
                DebugIM.println("BuddyList, changeBuddyStatus(), the status (" + stat + ") has" + " to be changed to "
                        + status + " for the buddy: " + buddy);
    ...
    {
        DebugIM.println("BuddyList, removeFromBuddyList(), the " + buddy + " is going " + " to be removed ");
