...
/**
 * This is a proxy default router. When the implementation wants to forward a
...
public class ProxyRouter implements Router
{
    ...
    {
        ProxyDebug.println("DEBUG, ProxyRouter, the default route is: " + def);
        if (def != null)
            this.defaultRoute = new ProxyHop(def);
        this.stack = sipStack;
    }

    public ProxyHop getNextHop(ListIterator routes) throws IllegalArgumentException
    {
        try
        {
            while (routes.hasNext())
            {
                ...
                ProxyDebug.println("DEBUG, ProxyRouter, getNextHop(), "
                        + "The request has at least one route, we use the first one" + ": " + host + ":" + port + "/"
                        + transport);
                return hop;
    ...
    public ListIterator getNextHops(Request request) throws IllegalArgumentException
    {
        ...
        if (requestURI instanceof SipURI)
        {
            ...
            if (mAddr != null)
            {
                try
                {
                    ...
                    ProxyHop mAddrHop = new ProxyHop(mAddr, mAddrPort, mAddrTransport);
                    if (mAddrHop != null)
                    {
                        nextHops.add(mAddrHop);
                        ProxyDebug.println("DEBUG, ProxyRouter, getNextHops(), "
                                + " the hop added: Request URI maddr parameter:" + " host:" + mAddr + " port:"
                                + mAddrPort + " transport:" + mAddrTransport);
                        return nextHops.listIterator();
                    }

                } catch (Exception e)
                {
                    throw new IllegalArgumentException("ERROR, ProxyRouter, pb to add the maddr hop");
                }
            } else
            {
                ...
                ProxyDebug.println("DEBUG, ProxyRouter, getNextHops(), " + " the hop added: host:"
                        + requestURIHop.getHost() + " port:" + requestURIHop.getPort() + " transport:"
                        + requestURIHop.getTransport());
                return nextHops.listIterator();

            }
        } else
        {
            ProxyDebug.println("DEBUG, ProxyRouter, getNextHops(), the request URI is not a SipURI:"
        ...
        if (defaultRoute != null)
        {
            ...
            ProxyDebug.println("DEBUG, ProxyRouter, getNextHops(), we added the hop (default route): " + "host:"
                    + defaultRoute.getHost() + " port:" + defaultRoute.getPort() + " transport:"
                    + defaultRoute.getTransport());
        }
