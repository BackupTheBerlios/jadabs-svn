...
public class RequestForwarding
{
    ...
    public void forwardRequest(Vector targetsURIList, SipProvider sipProvider, Request request,
            ServerTransaction serverTransaction, boolean statefullForwarding)
    {
        /*
        ...
        try
        {
            ...
            for (int i = 0; i < targetsURIList.size(); i++)
            {
                ...
                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequest() (STEP 1)," + " the request is cloned");
                /** ************************************************************************ */
                /** ************ 2. Update the Request-URI ********* */
                /** ************************************************************************ */
                ...
                if (requestURI.isSipURI())
                {
                    if (proxy.managesDomain(((SipURI) requestURI).getHost()))
                    {
                        ...
                        if (ProxyDebug.debug)
                        {
                            ProxyDebug.println("RequestForwarding, forwardRequest() (STEP 2),"
                    ...
                    } else
                    {
                        ...
                        {
                            ProxyDebug.println("RequestForwarding, forwardRequest() (STEP 2),"
                                    + "the proxy does not manage the domain " + ((SipURI) targetURI).getHost());
                        }
                    }
                }
                ...
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("RequestForwarding, forwardRequest() " + " (STEP 3), MaxForwardHeader "
                ...
                if (statefullForwarding)
                {
                    ...
                    if (ProxyDebug.debug)
                        ProxyDebug.println("RequestForwarding, forwardRequest(), (STEP 4)"
                                + " record-route header created and added to the " + " cloned request");
                } else
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("RequestForwarding, forwardRequest(), (STEP 4)"
                                + " record-route header not added to the cloned request " + " (stateless)");
                }
                ...
                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequest(), (STEP 5)"
                            + " No Additional headers to add...");
                ...
                if (routes != null && routes.hasNext())
                {
                    ...
                    if (routeURI.isSipURI() && ((SipURI) routeURI).hasLrParam())
                    {
                        ...
                        if (stackIPAddress.equals(host))
                        {
                            ...
                            if (ProxyDebug.debug)
                                ProxyDebug.println("Proxy, forwardRequest(), (STEP 6) we removed"
                    ...
                    } else if (ProxyDebug.debug)
                        ProxyDebug.println("RequestForwarding, forwardRequest(), (STEP 6)"
                                + " no Postprocess routing information to do " + "(the route has no lr parameter)...");
                } else
                {
                ...
                /** **************************************************************************** */
                /**
                 * ************ 7. Determine Next-Hop Address, Port, and
                 * Transport *********
                 */
                /** **************************************************************************** */
                ...
                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequest(), (STEP 8)"
                ...
                try
                {
                    ...
                    {
                        if (ProxyDebug.debug)
                            ProxyDebug.println("RequestForwarding, forwardRequest()," + " no Content-Type header,"
                ...
                {
                    ProxyDebug.println("RequestForwarding, forwardRequest(), Stripe" + " Parameter failed!!!!");
                }
        ...
        } catch (Exception ex)
        {
            try
            {
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("RequestForwarding, forwardRequest(), " + " internal error, "
                            + "exception raised:");
    ...
    private void forwardRequestStatefully(SipProvider sipProvider, Request clonedRequest, Request originalRequest,
            ServerTransaction serverTransaction)
    {
        ...
        if (ProxyDebug.debug)
        {
            ProxyDebug.println("serverTransaction =  " + serverTransaction);
            if (serverTransaction != null)
                ProxyDebug.println("dialog =  " + serverTransaction.getDialog());
        }
        ...
        try
        {
            ...
            if (serverTransaction == null && !clonedRequest.getMethod().equals(Request.MESSAGE))
            {
                ...
                // client transaction.
                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequestStatefully(),"
                            + " the cloned request does not have a server transaction," + " so we drop the request!");
                return;
            }
            ...
            if (originalRequest.getMethod().equals(Request.CANCEL))
            {
                ...
                if (ProxyDebug.debug)
                    ProxyDebug.println("Proxy, processRequest(), 487 Request Terminated" + " replied to the CANCEL:\n"
                            + response.toString());
            }
            ...
            if (dialogState == null)
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequestStatefully(),"
                            + " the dialog state is null, so we have to" + " forward the request using"
            ...
            /*
             * // Special processing because the SUBSCRIBE is // hop by hop for
             * a presence server if ( proxy.isPresenceServer() ) {
             * this.forwardIMPresenceRequest ( sipProvider, serverTransaction,
             * clonedRequest, originalRequest, dialog); return; } else {
             */

            this.forwardRequestThroughDialog(sipProvider, serverTransaction, clonedRequest, dialog);
            return;
        ...
        } catch (Exception ex)
        {
            try
            {
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("RequestForwarding, forwardRequestStatefully(), " + " internal error, "
                            + "exception raised:");
                    ProxyDebug.logException(ex);
                    ex.printStackTrace();
                    System.exit(0);
    ...
    /**
     * Special processing for Instant Messaging and Presence messages.
     * 
     * @deprecated No special processing needed for SUBSCRIBE, either this
     *             presenceserver is the PA of the PUA and the subscribe ends
     *             here, or the subscribe is forwarded like any other message.
     */

    private void forwardIMPresenceRequest(SipProvider sipProvider, ServerTransaction serverTransaction,
            Request clonedRequest, Request originalRequest, Dialog dialog) throws Exception
    { /*
       * TransactionsMapping transactionsMapping= (TransactionsMapping)
       * serverTransaction.getDialog(). getApplicationData(); CSeqHeader
       * cseqHeader = (CSeqHeader) clonedRequest.getHeader(CSeqHeader.NAME); if (
       * cseqHeader.getSequenceNumber()==1 &&
       * clonedRequest.getMethod().equals(Request.SUBSCRIBE) ){ if
       * (ProxyDebug.debug) ProxyDebug.println ("RequestForwarding,
       * forwardRequestStatefully(),"+ " It is a first SUBSCRIBE, so we have
       * to"+ " forward the request using"+ " a new clientTransaction");
       * ClientTransaction clientTransaction =
       * sipProvider.getNewClientTransaction (clonedRequest);
       * clientTransaction.sendRequest();
       * 
       * if (ProxyDebug.debug) ProxyDebug.println ("RequestForwarding,
       * forwardRequestStatefully(),"+ ", cloned request forwarded statefully:\n "
       * +clonedRequest);
       * 
       * if (serverTransaction.getDialog() != null) {
       * transactionsMapping.addMapping (serverTransaction,clientTransaction); }
       * return; } else if (clonedRequest.getMethod().equals(Request.NOTIFY)) { //
       * Forward the notify to all the watchers. String url = ( (FromHeader)
       * clonedRequest.getHeader(FromHeader.NAME)).
       * getAddress().getURI().toString(); if(url.indexOf(';')!=-1)
       * url=url.substring(0,url.indexOf(';')); PresenceServer ps =
       * proxy.getPresenceServer(); PresentityManager pm =
       * ps.getPresentityManager(); Vector presentities =
       * pm.getSubscriberList(url); //System.out.println("GOT A NOTIFY...." +
       * presentities.size()); // get all the dialogs for the watchers. for (int
       * i = 0; i < presentities.size() ; i++) { Subscriber s = (Subscriber)
       * presentities.elementAt(i); if (ProxyDebug.debug) { ProxyDebug.println
       * ("Forwarding request to Subscriber nb "+ i+ " :
       * "+s.getSubscriberURL()); } Dialog d = s.getDialog(); Request r =
       * d.createRequest(Request.NOTIFY); // set the content of the outgoing
       * request. Object content=clonedRequest.getContent(); if(content!=null)
       * r.setContent(content, (ContentTypeHeader) clonedRequest.getHeader
       * (ContentTypeHeader.NAME)); ClientTransaction ct =
       * sipProvider.getNewClientTransaction (r); d.sendRequest(ct); } return; }
       * else if (clonedRequest.getMethod().equals(Request.MESSAGE)) { if
       * (dialog != null) { this.forwardRequestThroughDialog(sipProvider,
       * serverTransaction, clonedRequest, dialog); } else {
       * 
       * forwardRequestStatelessly(sipProvider, clonedRequest, originalRequest,
       * serverTransaction);
       *  // ClientTransaction clientTransaction = //
       * sipProvider.getNewClientTransaction // (clonedRequest); //
       * clientTransaction.sendRequest(); } } //Means that the message is not a
       * presnece message else{ this.forwardRequestThroughDialog( sipProvider,
       * serverTransaction, clonedRequest, dialog); }
       */
    }
     ...
     */
    private void forwardRequestThroughDialog(SipProvider sipProvider, ServerTransaction serverTransaction,
            Request clonedRequest, Dialog dialog) throws Exception
    {
        TransactionsMapping transactionsMapping = (TransactionsMapping) serverTransaction.getDialog()
                .getApplicationData();
        ...
        if (clonedRequest.getMethod().equals("CANCEL"))
        {
            ...
            if (firstTransaction == null) { throw new Exception("ERROR, RequestForwarding, the first transaction"
            ...
            if (clientTransactions == null || clientTransactions.isEmpty()) { throw new Exception(
                    "RequestForwarding, The peer first client " + " transaction(s) for the first server transaction "
                            + " is null (CANCEL)"); }

            for (Enumeration e = clientTransactions.elements(); e.hasMoreElements();)
            {
                ...
                ClientTransaction clientTransaction = sipProvider.getNewClientTransaction(cancelRequest);
                clientTransaction.sendRequest();

                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequestStatefully()"
                            + ", created CANCEL sent statefully (using a new client" + " transaction):\n "
                            + cancelRequest);
            }
            return;
        }
        ...
        if (peerDialog == null)
        {
            ClientTransaction ct = sipProvider.getNewClientTransaction(clonedRequest);
            ct.sendRequest();
        ...
        } else if (clonedRequest.getMethod().equals("ACK"))
        {
            ...
            if (ProxyDebug.debug)
                ProxyDebug.println("RequestForwarding, forwardRequestStatefully()"
                        + ", cloned ACK forwarded statefully " + " (using the dialog from the"
                        + " first client transaction):\n " + clonedRequest);

        } else
        {

            Request dialogRequest = peerDialog.createRequest(clonedRequest.getMethod());
            ...
            } catch (Exception e)
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("RequestForwarding, forwardRequestStatefully()"
                            + ", error trying to copy the headers from " + " the original request");
                e.printStackTrace();
            }
            if (ProxyDebug.debug)
            {
            ...
            if (ProxyDebug.debug)
                ProxyDebug.println("RequestForwarding, forwardRequestStatefully()"
                        + ", cloned request forwarded statefully " + "(using the dialog from the"
                        + " first client transaction):\n " + dialogRequest);
            transactionsMapping.addMapping(serverTransaction, clientTransaction);
        }
    }

    private void forwardRequestStatelessly(SipProvider sipProvider, Request clonedRequest, Request originalRequest,
            ServerTransaction serverTransaction)
    {
        ...
        try
        {
            ...
            if (ProxyDebug.debug)
                ProxyDebug.println("RequestForwarding, forwardRequestStatelessly(), "
        ...
        } catch (Exception ex)
        {
            try
            {
                if (ProxyDebug.debug)
                {
                    ProxyDebug.println("RequestForwarding, forwardRequestStatelessly(), " + " internal error, "
                            + "exception raised:");
                    ProxyDebug.logException(ex);
                }
                // This is an internal error:
