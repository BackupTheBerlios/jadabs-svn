...
/**
 * The subscriber object contains information of a subscriber and the
 * subscription. It keeps track of creation and expires times and the dialog in
 * which the subscription exists. The object also buffers the next notification
 * body so that several notifiers may update it.
 * 
 * This object is subclassed for use in specific event packages. Note that Java
 * uses dynamic method lookup, which means that the correct overridden method is
 * used. Very handy.
 * 
...
public class Subscriber
{
     ...
     * 
     * @param newString
     *            Must be a valid message, so that it can be put into a Notify
     *            request directly.
     */
    public void updateNotifyBody(String newString)
    {

        //Should be able to merge notifyBodies. See
        // PresentityManager:processPublish()
