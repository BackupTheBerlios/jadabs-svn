...
/**
 * A virtual subscription is a subscription that the Resource List Server
 * (colocated with the Presence Server in this implementation) issues if someone
 * subscribes to a resource list containing external references.
 * <p />
 * VirtualSubscriptions are sent to one PA, and thus what we receive is the full
 * pidf for all the PA's PUAs (in other words, if the notifier we are
 * subscribing to has a presenceServer like this one, the presenceServer will
...
public class VirtualSubscription
{
    ...
    /**
     * Collects the extra time that the subscribers require. Used when
     * resubscribing to th virtualSubscription *
     */
    ...
    /**
     * Adds a new subscriber and sets the extraTime field to this subscribers
     * expires-time if larger. This means that the newExpires field will contain
     * a sufficiently large number when resubscribing to the notifier.
     *  
     */
    protected void addSubscriber(Subscriber subscriber, String subscriberKey, int subscribeExpires)
    {

        //Does this subscription need more time than what we have subscribed
        // for?
     ...
     * longer than what is negotiated, the extraTime field is larger than zero.
     */
