...
public class ResponseForwarding
{
    ...
    protected void forwardResponse(SipProvider sipProvider, Response response, ClientTransaction clientTransaction)
    {
        ...
        try
        {
            ...
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("ResponseForwarding, forwardResponse(), "
                            + " the client transaction does not exist, " + " will forward the response statelessly!");
                ...
                if (ProxyUtilities.hasTopViaHeaderProxy(sipStack, response))
                {
                    ...
                    {
                        if (ProxyDebug.debug)
                            ProxyDebug.println("ResponseForwarding, forwardResponse(), " + "the response does "
                                    + " not have any Via header, the response is " + "silently discarded");
                    } else
                    {
            ...
            if (!viaList.hasNext())
            {
                ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 3)," + " the response does not "
            ...
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 5)"
            ...
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 5)," + " we don't forward the"
                            + " 487 Request terminated, in statefull behavior " + " (it's for the proxy).");
                return;
            }
            ...
            if (proxy.isPresenceServer() && response.getStatusCode() == 200 && cseqHeader.getMethod().equals("NOTIFY"))
            {
                if (ProxyDebug.debug)
                    ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 5),"
            ...
            if ((response.getStatusCode() == 401 || response.getStatusCode() == 603) && serverTransaction != null)
            {
                ...
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 9), "
                                + "the BUSY or DECLINE corresponds to a forked call, " + " so we drop it and wait for"
            ...
            if (response.getStatusCode() == 200 && serverTransaction != null)
            {
                ...
                if (peerDialog != null && peerDialog.getState() != null
                        && peerDialog.getState().equals(DialogState.CONFIRMED) && clientsTransactionList != null
                        && clientsTransactionList.size() > 1)
                {
                    if (ProxyDebug.debug)
                        ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 9), "
                                + "the 200 OK corresponds to a forked call, " + "so we drop it and send a BYE");
            ...
            if (serverTransaction == null)
            {
                sipProvider.sendResponse(response);
                if (ProxyDebug.debug)
                    ProxyDebug.println("ResponseForwarding, forwardResponse() (STEP 9), the "
                            + " response is statelessly" + " forwarded:\n" + response.toString());
                return;
            } else
            {
                ...
                if (dialog != null)
                {
                    ...
                    if (localTag != null && remoteTag != null)
                    {
                        if (dialog.isServer())
                        {
                            ...
                            if (ProxyDebug.debug)
                                ProxyDebug.println("ResponseForwarding, forwardResponse() "
                                        + " (STEP 9), we changed the " + "toTag:" + localTag);
                        } else
                        {
                            toHeader.setTag(localTag);
                            if (ProxyDebug.debug)
                                ProxyDebug.println("ResponseForwarding, forwardResponse() "
                                        + " (STEP 9),  we changed the fromTag:" + remoteTag);
                        }
                    }
                }
            ...
            if (response.getStatusCode() == 200 || (response.getStatusCode() >= 600 && response.getStatusCode() <= 606))
            {
                Vector clientsTransactionList = transactionsMapping.getClientTransactions(serverTransaction);
                ...
                for (Enumeration e = clientsTransactionList.elements(); e.hasMoreElements();)
                {
                    ...
                    if (ctr != clientTransaction)
                    {
                        ...
                        if (transactionState == null
                                || transactionState.getValue() == TransactionState.PROCEEDING.getValue())
                        {
                            ...
                            if (ProxyDebug.debug)
                                ProxyDebug.println("ResponseForwarding, forwardResponse()"
                                        + " (STEP 10), CANCEL created" + " and sent for cancel the pending "
        ...
        } catch (Exception ex)
        {
            if (ProxyDebug.debug)
            {
                ProxyDebug.println("ResponseForwarding, forwardResponse(), internal error, " + "exception raised:");
                ProxyDebug.logException(ex);
            }
        } finally
        {
            if (clientTransaction != null && clientTransaction.getState().equals(TransactionState.COMPLETED))
            {
                if (clientTransaction.getDialog() != null)
                {
                    transactionsMapping = (TransactionsMapping) clientTransaction.getDialog().getApplicationData();
