...
/**
 * The notifier object represents a registered sip user. It keeps track of all
 * the users UAs (contacts)
 * 
 * <h3>Signing off</h3>
 * When a presentity logs off, the object is active until all subscribers have
 * been notified and been removed from the list of subscribers.
 * 
 * 
 * @author Henrik Leion
 * @version 0.1
...
public class Notifier
{
    ...
    /***************************************************************************
     * Maps the subscriber URIs of this notifier to dialogIds. These dialogIds
     * are used by the presentityManager to find the correct subscriber when the
     * notifier wants to update it
     **************************************************************************/
    ...
    /*
     * A possible record for keeping event states . private class EventRecord {
     * protected String event; protected String entity; protected String state;
     * protected String contact protected int expires; protected long
     * creationTime;
     * 
     * public EventRecord(String ev, String en, String st, String ct, int ex) {
     * this.event=ev; this.entity=en; this.state=st; this.contact=ct;
     * this.expires=ex; this.creationTime=System.currentTimeMillis(); } }
     */
    ...
    public Notifier(String uri, int exp, String contact)
    {
        ...
        //User directory
        String username = uri.substring(uri.indexOf("sip:") + 4, uri.indexOf("@"));
        this.userDir = "users" + File.separator + username + File.separator;

        //Watcher info
        String watcherText = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                + " <watcherinfo version=\"0\" state=\"full\">\n" + "  <watcher-list resource=\"" + uri
                + "\" package=\"presence\">\n" + "  </watcher-list>\n" + " </watcherinfo>";
        ...
        this.fullState = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                + "<presence xmlns=\"urn:ietf:params:xml:ns:pidf\"" + " entity=\"" + notifierURI + "\">\n"
                + "</presence>";
    ...
    protected int addSubscriber(String subscriberKey, String dialogId, String status)
    {
        ...
        //write Winfo
        //new subscriber
        String text = "watcher id=\"" + subscriberKey + "\" status=\"" + status + "\" event=\"subscribe\">"
     ...
     * <li>The storage of event states is currently in a pidf-string (only
     * presence), a better solution is some sort of record or database entry
     * with <entity,event,state,expiration>, and generate the xml-code from
     * this. <br />
    ...
    /**
     * One of the notifiers UA has deregistered so it must be removed and the
     * fullState document must be updated.
     * 
    ...
    protected String removeContact(String contact)
    {
        ...
        /*
         * There is no guarante that there is a known link between a tuple id
         * and a contact URI. This means that a de-register cannot always find
         * and update a removed contact. The clients should use Publish to
         * update their presence status.
         * 
         * Strange exception when parsing contact-elements. Commented out it
         */

        //System.out.println("Notifier: removeContact - Trying to remove " +
        // contact +
        //		   " from:\n " + fullState);
        ...
        //System.out.println("Notifier: removeContact - found contact at " +
        // contactIndex);
        if (contactIndex > 0)
        {
            int tupleStart = fullState.indexOf("<tuple"); //at least one tuple
                                                          // always exist
            int tupleEnd = 0;
            ...
            while (tupleStart != -1)
            {
                ...
                tupleStart = fullState.indexOf("<tuple", tupleEnd);//where the
                                                                   // last tuple
                                                                   // ended
                ...
                { //tupleStart<contactIndex<tupleEnd
    ...
    protected boolean hasEvent(String event)
    {
        ...
        /*
         * Iterator it = allowEventsHeaders.iterator(); while (it.hasNext()) {
         * String temp = (String) it.next(); if (temp.equalsIgnoreCase(event))
