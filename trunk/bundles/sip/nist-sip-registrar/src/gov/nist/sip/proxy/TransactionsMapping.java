...
class TransactionsMapping
{
    ...
    protected ServerTransaction getServerTransaction(ClientTransaction clientTransaction)
    {
        ...
        while (e.hasMoreElements())
        {
            ...
            for (Enumeration en = vector.elements(); en.hasMoreElements();)
            {
                ...
                if (ct == clientTransaction) { return serverTransaction; }
            }
        }
        return null;
    }
    ...
    protected ClientTransaction getClientTransaction(ServerTransaction serverTransaction)
    {
        ...
        else
        {
            for (Enumeration e = vector.elements(); e.hasMoreElements();)
            {
                ...
                if (d.getState() != null && d.getState().equals(DialogState.CONFIRMED))
                    return ct;
                else if (d.getState() == null)
                {
                    ProxyDebug.println("TransactionsMapping, getClientTransaction()," + " the dialog state is null.");
                } else
                {
                    ProxyDebug.println("TransactionsMapping, getClientTransaction()," + " the dialog state is:"
                            + d.getState().toString());
                }
            }
            return null;
    ...
    {
        if (clientTransaction == null || clientTransaction.getDialog() == null) { return; }
        Vector clients = getClientTransactions(serverTransaction);
        Dialog dialog = serverTransaction.getDialog();
        TransactionsMapping map = (TransactionsMapping) dialog.getApplicationData();
        ...
        synchronized (table)
        {
            ...
            while (e.hasMoreElements())
            {
                ...
                if (serverTransaction.getDialog() != null)
                    ProxyDebug.println("   - dialog state: " + serverTransaction.getDialog().getState());
                ...
                if (vector != null)
                {
                    ...
                    {
                        ClientTransaction clientTransac = (ClientTransaction) vector.elementAt(i);
                        ProxyDebug.println("   - its clientTransaction: " + clientTransac + ", state: "
                                + serverTransaction.getState());
                    }
                }
            }
