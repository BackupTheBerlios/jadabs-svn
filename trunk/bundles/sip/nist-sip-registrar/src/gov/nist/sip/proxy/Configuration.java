...
public class Configuration
{
    ...
    public boolean hasDomain(String domainParam)
    {
        if (domainList != null)
            for (int j = 0; j < domainList.size(); j++)
            {
                ...
                if (domain.equalsIgnoreCase(domainParam))
                    return true;
            }
        return false;
    }
    ...
    public void addListeningPoint(String port, String transport)
    {
        ...
        {
            counter++;
            listeningPoints.put("listeningPoint" + counter, new Association(port, transport));
        }
    }
    ...
    public boolean isValidConfiguration()
    {
        ...
        {
            if (check(method) && check(classFile) && check(passwordsFile))
            {
                // OK
            } else
            {
                System.out.println("ERROR, the configuration is not valid: Problem with"
                        + " the authentication tag and authentication parameters");
                return false;
            }
        }

        if (check(stackName) && check(stackIPAddress) && !listeningPoints.isEmpty())
        {
            return true;
        } else
        {
